...
function getListContractToken(address) {
  return axios
    .get(
      process.env.URL_NEAR_API + 'account/' + address + '/likelyTokensFromBlock'
    )
    .then((response) => {
...
async function getTokenBalance({ contract, address, symbol }) {
  ...
  return account
    .viewFunctionV1(contract, 'ft_balance_of', { account_id: address })
    .then((response) => {
      return {
        balance: response,
        price: priceToken,
...
async function getTokenMetadata(contract) {
  const account = await walletUtils.nearConnection()
  return account.viewFunctionV1(contract, 'ft_metadata')
}
    ...
    try {
      await getTokenMetadata(listContract[i]).then(async (metadata) => {
        ...
        if (Number(balance) > 0) {
          const balanceUsd = (
            Number(
              walletUtils.formatTokenAmount(balance, metadata.decimals, 5)
            ) * Number(price)
          ).toFixed(2)
          list.push({
            contract: listContract[i],
            balance: walletUtils.formatTokenAmount(
              balance,
              metadata.decimals,
              5
            ),
            ...
            icon: metadata.icon,
            balance_usd: isNaN(balanceUsd) ? 0.0 : balanceUsd,
            price,
          })
        }
      })
...
async function getInventoryUser() {
  ...
  await axios
    .get(`${process.env.URL_API_INDEXER}/account/${address}/inventory`)
    .then((response) => {
      ...
      list.fts = list.fts.concat(
        data.fts.map((item) => {
          const price = !item.ft_metas.price ? 0 : item.ft_metas.price
          const balance = Number(
            walletUtils.formatTokenAmount(
              item.amount,
              item.ft_metas.decimals,
              5
            )
          )
