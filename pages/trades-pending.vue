<template>
  <div id="trades-pending" class="d-flex flex-column">
    ...
    <aside style="display: grid; grid-template-columns: repeat(5, 1fr) 35px; grid-template-rows: repeat(3, 1fr); gap: 15px;">
      <v-card class="card pa-4 px-6 d-flex" style="--bg: var(--card); grid-column: span 4; grid-row: span 2; gap: 5px;">
        <div>
      ...
      <v-card class="card clock pa-4" style="--bg: var(--card); grid-column: span 2; grid-row: span 2;">
        {{ formattedTime(secondsEstimated) }}
      </v-card>

      <v-card class="card px-4 d-flex align-center justify-space-between" style="--bg: var(--card); grid-column: span 5">
        <profile-avatar
          show-name
          :profile="{
            name: 'Star_Tranding',
    ...
    <aside class="my-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 151px), 1fr)); grid-auto-rows: 111px; gap: 15px;">
      ...
      <v-card class="card flex-grow-1 d-flex flex-column pa-4" style="--bg: var(--card)">
    ...
    <button
      class="mt-6"
      @click="$router.push('/trades-check')"
    >this button is only to navigate to trades check view, must to be deleted when integration starts</button>
  </div>
</template>
